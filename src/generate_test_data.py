...
def generate_test_file(output_file, total_ops, read_ratio=0.5, seed=0):
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python generate_test_data.py <output_file> <total_operations> [read_ratio] [seed]"
        )
        print("Example: python generate_test_data.py test_data.txt 1000000 0.5 42")
        sys.exit(1)
    ...
    generate_test_file(output_file, total_ops, read_ratio, seed)
